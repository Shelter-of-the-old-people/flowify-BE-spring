        ...
        try {
            ...
            if (response == null || response.get("access_token") == null) {
                throw new BusinessException(ErrorCode.OAUTH_TOKEN_EXPIRED,
                        "Google Drive 토큰 갱신 응답이 유효하지 않습니다. 재연결이 필요합니다.");
            }
            ...
            log.warn("Google OAuth token refresh failed: status={}, body={}",
                    e.getStatusCode().value(), e.getResponseBodyAsString());
            throw new BusinessException(ErrorCode.OAUTH_TOKEN_EXPIRED,
                    "Google Drive 토큰 갱신에 실패했습니다. 재연결이 필요합니다.");
        } catch (BusinessException e) {
            throw e;
        } catch (Exception e) {
            log.warn("Google OAuth token refresh failed", e);
            throw new BusinessException(ErrorCode.OAUTH_TOKEN_EXPIRED,
                    "Google Drive 토큰 갱신에 실패했습니다. 재연결이 필요합니다.");
        }
    }
