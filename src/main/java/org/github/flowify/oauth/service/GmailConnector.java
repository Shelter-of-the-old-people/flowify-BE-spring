...
public class GmailConnector implements ExternalServiceConnector {
    ...
    @Value("${app.oauth.gmail.redirect-uri}")
    private String redirectUri;

    @Value("${app.oauth.gmail.scopes}")
    ...
    public void handleCallback(String code, String state) {
        ...
        if (response == null || !response.containsKey("access_token")) {
            ...
            log.error("Gmail token exchange failed: {}", error);
            throw new BusinessException(ErrorCode.AUTH_OAUTH_FAILED,
                    "Gmail 토큰 교환에 실패했습니다: " + error);
        }
        ...
        oauthTokenService.saveToken(userId, "gmail", accessToken, refreshToken, expiresAt, tokenScopes);

        log.info("Gmail OAuth token saved for userId={}", userId);
    }
}
